...
contract TOKEN is Context, IBEP20, Ownable {
    ...
    constructor (address cOwner, address marketingWlt) Ownable(cOwner) {
        ...
        // uniswap
        IzData zData = IzData(0x544cc0eA0419a0E7586D8BF24547077dDdDe99f2);
        _usdt = zData.USDT();
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        */
        ...
        if (marketingFee > 0) {
            swapForUsdt(marketingFee);
            uint256 usdtBalance = IBEP20(_usdt).balanceOf(address(this));
            _approve(address(this), _marketingWallet, usdtBalance);
            IBEP20(_usdt).transfer(_marketingWallet, usdtBalance);
            // payable(_marketingWallet).transfer(marketingFee);
            emit MarketingFeeSent(_marketingWallet, usdtBalance);
        }
    ...
    function swapForUsdt(uint256 bnbAmount) private {
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
